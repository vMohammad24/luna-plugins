...
import { fetchIsrcIterable } from "../../fetch/tidalApi";
...
export class MediaItem extends ContentBase {
	...
	public brainzItem: () => Promise<ITrack | undefined> = memoize(async () => {
		const releaseTrackFromRecording = async (recording: IRecording) => {
			...
			const release = await fetchJson<IRecording>(`https://musicbrainz.org/ws/2/recording/${recording.id}?inc=releases+media+artist-credits+isrcs&fmt=json`)
		...
		if (this.tidalItem.isrc !== undefined) {
			...
			const recording = await fetchJson<{ recordings: IRecording[] }>(`https://musicbrainz.org/ws/2/isrc/${this.tidalItem.isrc}?inc=isrcs&fmt=json`)
