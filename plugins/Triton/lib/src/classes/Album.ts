...
export class Album extends ContentBase implements MediaCollection {
	...
	public brainzAlbum: () => Promise<IReleaseMatch | undefined> = memoize(async () => {
		...
		const brainzAlbum = await fetchJson<{ releases: IReleaseMatch[] }>(brainzUrl)
	...
	public brainzRelease: () => Promise<IRelease | undefined> = memoize(async () => {
		...
		return fetchJson<IRelease>(`https://musicbrainz.org/ws/2/release/${brainzAlbum.id}?inc=recordings+isrcs+artist-credits&fmt=json`).catch(trace.warn.withContext("getReleaseAlbum"));
	});
